...
func init() {
	android.RegisterModuleType("fortune_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = fortuneExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = fortuneExpandVariables(ctx, dep_file)
	...
	cmd := fortuneExpandVariables(ctx, String(g.properties.Cmd))
