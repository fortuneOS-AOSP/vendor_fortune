...
fortune_local_manifest = ".repo/local_manifests/fortune.xml"
fortune_default_revision = "vangelis"
fortune_default_revision =  os.getenv('ROOMSERVICE_DEFAULT_BRANCH', 'vangelis')
fortune_dependencies = "fortune.dependencies"
org_manifest = "fortune-devices"  # leave empty if org is provided in manifest
...
def get_revision(manifest=None, p="build"):
    return fortune_default_revision

def get_from_manifest(device_name):
    if os.path.exists(fortune_local_manifest):
        man = load_manifest(fortune_local_manifest)
...
def is_in_manifest(project_path):
    for local_path in load_manifest(fortune_local_manifest).findall("project"):
...
def add_to_manifest(repos, fallback_branch=None):
    lm = load_manifest(fortune_local_manifest)
    ...
    for repo in repos:
        ...
        if 'branch' in repo:
            ...
            repo_branch = fortune_default_revision
    ...
    f = open(fortune_local_manifest, 'w')
    f.write(raw_xml)
    f.close()
...
def fetch_dependencies(repo_path, fallback_branch=None):
    ...
    dep_p = '/'.join((repo_path, fortune_dependencies))
    ...
    for dependency in dependencies:
        if not is_in_manifest(dependency['target_path']):
            if not dependency.get('branch'):
                dependency['branch'] = (
                    fallback_branch or get_revision() or fortune_default_revision
                )
...
def detect_revision(repo):
    ...
    if has_branch(result, fortune_default_revision):
        print("Falling back to custom revision: %s"
              % fortune_default_revision)
        return fortune_default_revision
